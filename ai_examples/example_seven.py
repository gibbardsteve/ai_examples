...
page_icon = (
    ":brain:"  # https://streamlit-emoji-shortcodes-streamlit-app-gwckff.streamlit.app/
)
layout = "centered"
# --------------------
...
# Initialise the chatbot message
...
# ----- CHAT INTERFACE ---------
# Write the historical chat messages, cater for defined avatar
...
# Assign prompt from the chat input and check it is not falsy
if prompt := st.chat_input():
    if not openai.api_key:
        st.info("Open AI Key Not Found")
        st.stop()
    ...
    response = ask_question(prompt, st.session_state.messages, "gpt-3.5-turbo")
